...
class EvaluationTests(unittest.TestCase):
    ...
    def evidence(self,user_id,timing,message_id):
        stamp="2026-07-23T18:00:00-04:00" if timing=="on_time" else "2026-07-24T18:00:00-04:00"
...
class JobAndReportTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_post_grace_run_sends_one_summary_across_repeats(self):
        ...
        with patch.object(db,"DATABASE_PATH",self.path),patch("tracker.send_routed",routed):
            first=await missing_pop_case_job(self.ctx,datetime(2026,7,25,0,5,tzinfo=ET))
            second=await missing_pop_case_job(self.ctx,datetime(2026,7,25,0,20,tzinfo=ET))
    ...
    async def test_all_clear_is_sent_at_most_once(self):
        ...
        routed=AsyncMock(return_value=(True,None))
        with patch.object(db,"DATABASE_PATH",self.path),patch("tracker.send_routed",routed):
            await missing_pop_case_job(self.ctx,datetime(2026,7,25,1,tzinfo=ET));await missing_pop_case_job(self.ctx,datetime(2026,7,25,2,tzinfo=ET))
    ...
    async def test_pop_report_lists_named_case_state(self):
        ...
        with patch.object(db,"DATABASE_PATH",self.path),patch("tracker.require_permission",AsyncMock(return_value=True)):
